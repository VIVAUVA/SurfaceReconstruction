function [st p] = surf_tpaps(x,y,p)
...
%   $Revision: 1.1 $  $Date: 2012-01-24 19:29:24 $
...
if nx==3 % simply return the interpolating plane
   ...
   elseif nx<100000 % we solve the linear system directly:
      ...
      for i=dy:-1:1
         ...
         [coefs1(:,i),flag] = gmres(@tppval,(y(i,:)*Q).',10, ...
                    1.e-6*max(abs(y(:))),[10],[],[],zeros(nx-3,1), st0,Q,p);
